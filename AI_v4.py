...
def minimax(board, depth, maxPlayer, a = float('-inf'), b = float('inf')):
  ...
  # move order so we do best moves first
  # increase3s alpha beta effectiveness
  abm = [3,2,4,1,5,0,6]
  if maxPlayer:
    score = float('-inf')
    bestMove = 0
    for x in abm:
      ...
      a = max(a, score)
      potential = minimax(move,depth - 1,False,a,b)[0]
      if potential > score:
        score = potential
        bestMove = x
      if score >= b:
    ...
    for x in abm:
      ...
      move = newMove(-1,x,board)
      b = min(b, score)
      potential = minimax(move,depth - 1,True,a,b)[0]
      if potential < score:
        score = potential
        bestMove = x
      if score <= a:
